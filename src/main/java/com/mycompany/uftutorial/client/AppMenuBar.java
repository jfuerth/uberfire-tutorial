    ...
    Menus menus =
      MenuFactory.newTopLevelMenu("Screens")
        .menus()
          ...
          .menu("Hello Screen (extends)").respondsWith(makeGoToPlaceCommand(HelloWorldScreenExtends.class)).endMenu()
          .menu("RSVP Editor (programmatic)").respondsWith(makeGoToPlaceCommand(InvitationRsvpPlainWidgets.class)).endMenu()
          .menu("Mood Screen").respondsWith(makeGoToPlaceCommand(MoodScreen.class)).endMenu()
          .menu("RSVP Editor (Errai UI)").respondsWith(makeGoToPlaceCommand(InvitationRsvpErraiUI.class)).endMenu()
          .menu("RSVP Editor (MVP)").respondsWith(makeGoToPlaceCommand(InvitationRsvpPresenter.class)).endMenu()
        .endMenus()
      .endMenu()
